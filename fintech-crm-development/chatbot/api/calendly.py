...
class Client(object):
    ...
    def __init__(
        self, access_token=None, client_id=None, client_secret=None, redirect_uri=None
    ):
    ...
    def refresh_access_token(self, refresh_token):
        params = {"client_id": self.CLIENT_ID, "client_secret": self.CLIENT_SECRET}
        body = {"grant_type": "authorization_code", "refresh_token": refresh_token}
        return self.post("oauth/token", params=params, data=body, auth_url=True)
    ...
    def post(
        self, endpoint, params=None, data=None, headers=None, json=True, auth_url=False
    ):
    ...
    def request(
        ...
        if auth_url:
            ...
            return requests.request(
                method, self.URL + endpoint, params=params, headers=_headers, **kwargs
            )
...
"""
The commented code is a file to setup Calendly webhook
We have to create a webhook to get the event from Calendly
It will be used to send the message to the user
Note: calendly webhook setup only once no need for accessing token everytime
"""
...
#     "https://whatsappbot.techdomeaks.com/calendly",
#     ["invitee.created", "invitee.canceled"],
#     organization_uri,
#     user_uri,
#     "user",
# )
# webhooks = client.list_webhooks("user", organization_uri, user_uri)


def send_put_request_with_token():
    headers = {
        "Authorization": "Bearer " + settings.CALENDLY_TOKEN,
        "Content-Type": "application/json",
    }
    data = {
        "max_event_count": 1,
        "owner": settings.CALENDLY_OWNER,
        "owner_type": "EventType",
    }
    response = requests.post(
        settings.CALENDLY_SINGLE_USE_LINK, headers=headers, json=data
    )
