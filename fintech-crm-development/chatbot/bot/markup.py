...
class Inline_button:
    def __init__(self, text: str, button_id: str = None):
        self.button = {
            "type": "reply",
            "reply": {"id": button_id if button_id else text, "title": text},
        }
    ...
    def set_buttons(self, _buttons: Union[List[str], List[Inline_button]]):
        ...
        for i in _buttons:
            if isinstance(i, str):
                ...
                raise ValueError("str or Inline_button expected as button elements")
        return res

    def error_check(self):
        if len(self.inline_buttons) > 3 or len(self.inline_buttons) < 1:
            raise ValueError(
                "Inline_keyboard can only accept a minimum of 1 Inline_button item and a maximum of 3"
            )
        button_id_check = []
        button_text_check = []
        for i, button in enumerate(self.inline_buttons):
            if not isinstance(button, Inline_button):
                raise ValueError(
                    f"Item at position {i} of list argument expected to be a string or an instance of Inline_button"
                )
...
class List_item:
    def __init__(self, title: str, _id: str = None, description: str = None) -> None:
        ...
        self.item = {"id": self._id, "title": self.title}
        if description:
...
class List_section:
    ...
    def set_section(self):
        sections = {"title": self.title, "rows": [i.item for i in self.items_list]}
        return sections


class Inline_list(Reply_markup):
    def __init__(
        self, button_text: str, list_items: Union[List[List_item], List[List_section]]
    ):
    ...
    def error_check(self):
        if not isinstance(self.list_items, list):
            raise ValueError("The argument for list_items should be of type 'list'")
    ...
    def set_list(self):
        action = {
            ...
            "sections": [i.section for i in self.list_items]
            if isinstance(self.list_items[0], List_section)
            else [{"rows": [i.item for i in self.list_items]}],
        }
        return action
